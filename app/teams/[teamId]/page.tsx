...
export default async function TeamPage({ params }: { params: { teamId?: string } } | { params: Promise<{ teamId?: string }> }) {
  ...
  const isMember = session?.user?.email ? team.members.some((m: typeof team.members[number]) => m.user?.email === session.user.email) : false
  const isAdmin = session?.user?.email ? team.members.some((m: typeof team.members[number]) => m.user?.email === session.user.email && m.isAdmin) : false
