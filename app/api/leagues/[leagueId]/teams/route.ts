...
export async function GET(
  ...
  try {
    ...
    const teamsWithStats = await Promise.all(
      leagueTeams.map(async (leagueTeam: typeof leagueTeams[number]) => {
        ...
        const completedMatches = matches.filter((match: typeof matches[number]) => 
          match.homeScore !== null && 
          match.awayScore !== null
        )

        const wins = completedMatches.filter((match: typeof completedMatches[number]) => {
        ...
        const losses = completedMatches.filter((match: typeof completedMatches[number]) => {
          if (match.homeTeamId === teamId) return (match.homeScore || 0) < (match.awayScore || 0)
          if (match.awayTeamId === teamId) return (match.awayScore || 0) < (match.homeScore || 0)
          return false
        }).length

        const ties = completedMatches.filter((match: typeof completedMatches[number]) =>
          (match.homeScore || 0) === (match.awayScore || 0)
        ).length

        const upcomingMatches = matches.filter((match: typeof matches[number]) =>
        ...
        return {
          ...
          adminCount: leagueTeam.team.members.filter((m: typeof leagueTeam.team.members[number]) => m.isAdmin).length,
