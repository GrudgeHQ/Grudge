...
export async function POST(req: Request, context: any) {
  ...
  const isMember = match.team.members.some(m => m.userId === user.id)
  if (!isMember) {
    return NextResponse.json({ error: 'Only team members can provide availability' }, { status: 403 })
  }
  ...
  if ((status === 'MAYBE' || status === 'UNAVAILABLE') && existingAssignment) {
    ...
    // Get all team admins
    const teamAdmins = match.team.members.filter(m => m.isAdmin)
...
export async function GET(req: Request, context: any) {
  ...
  if (!match) {
    ...
    const userTeamMemberIds = userTeam.members.map(m => m.userId)
    ...
    const transformedAvailabilities = seasonAvailabilities.map(sa => ({
      id: sa.id,
  ...
  const membership = match.team.members.find(m => m.userId === user.id)
  if (!membership) {
