...
export async function POST(req: Request, { params }: { params: Promise<{ matchId: string; submissionId: string }> }) {
  ...
  const userTeamIds = user.memberships
    .filter((m: typeof user.memberships[number]) => m.isAdmin)
    .map((m: typeof user.memberships[number]) => m.teamId)
  ...
  updatedSubmission = await prisma.$transaction(async (tx) => {
      ...
      const idsToDelete = notificationsToDelete
        .filter((n: typeof notificationsToDelete[number]) => {
          const payload = n.payload as any
          return payload?.scoreSubmissionId === submissionId
        })
        .map((n: typeof notificationsToDelete[number]) => n.id)
      ...
      const idsToDeleteDispute = notificationsToDeleteDispute
        .filter((n: typeof notificationsToDeleteDispute[number]) => {
          const payload = n.payload as any
          return payload?.scoreSubmissionId === submissionId
        })
        .map((n: typeof notificationsToDeleteDispute[number]) => n.id)
