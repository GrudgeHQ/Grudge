...
export async function GET(req: Request, context: any) {
  ...
  const isHomeTeamMember = seasonMatch.homeTeam.members.some((m: typeof seasonMatch.homeTeam.members[number]) => m.userId === caller.id)
  const isAwayTeamMember = seasonMatch.awayTeam.members.some((m: typeof seasonMatch.awayTeam.members[number]) => m.userId === caller.id)
  ...
  const callerMembership = callerTeam.members.find((m: typeof callerTeam.members[number]) => m.userId === caller.id)
  ...
  const teamMemberIds = callerTeam.members.map((m: typeof callerTeam.members[number]) => m.userId)
